...
@login_required
def Home(request):
    # user authentication and login required
...
def Logout(request):
    #User logging out of the system
    logout(request)
    return HttpResponseRedirect('/')

def new(request):
    # new user request authenticated
    if request.user.is_authenticated:
...
def show(request, id):
    # show client id and details of a specified client
    if request.user.is_authenticated:
...
def edit(request,id):
    # show edit information through id
    if request.user.is_authenticated:
        client = Client.objects.get(id=id)
        context = {
            'client': client
...
def update(request):
    # update client information through id
    if request.user.is_authenticated:
        if request.POST:
...
def delete(request,id):
    # delete client information through id
    if request.user.is_authenticated:
        client = Client.objects.get(id=id)
        if request.user.id == client.admin.id:
